...
class CNNCategoricalActor2(nn.Module):
    
    def __init__(self, input_shape, act_dim, activation):
        super().__init__()
        self.input_shape = input_shape
        self.act_dim = act_dim 
        self.cnn_layer = CNNLayer2(input_shape)
        self.linear_layer = mlp([64]+[256]+[act_dim], activation)
        self.logits_net = nn.Sequential(self.cnn_layer, self.linear_layer)
...
class RLAgent2:
    ...
    def __init__(self, state_shape, action_shape):
        ...
        self.actor = CNNCategoricalActor2(state_shape, action_shape, nn.ReLU)
...
agent = RLAgent2(state_shape, action_shape)
